...
export abstract class ModelRouter<D extends mongose.Document> extends Router{
    constructor(protected model: mongose.Model<D>){
        super()
    }
    ...
    validateId = (req, resp, next)=>{
        if(!mongose.Types.ObjectId.isValid(req.params.id)){
            next(new NotFoundError('Id não encontrado'))
        }else{
            next()
        }
    }
